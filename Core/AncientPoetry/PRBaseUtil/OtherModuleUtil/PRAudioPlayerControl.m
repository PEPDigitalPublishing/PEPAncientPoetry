...
@interface PRAudioPlayerControl ()<AudioPlayerOnlineEngineDelegate, MZTimerLabelDelegate>
...
@implementation PRAudioPlayerControl
...
- (IBAction)progressSliderValueChanged:(UISlider *)sender {
    self.sliding = true;
    self.playShowTimeView.hidden = NO;
...
- (IBAction)speedControlAction:(UIControl *)sender {
    if (self.viewController.presentedViewController == self.speedAlertController) {
        ...
        [self.viewController presentViewController:self.speedAlertController animated:true completion:^{
...
- (IBAction)clickAudioListBtn:(id)sender {
    if ([self.delegate respondsToSelector:@selector(audioPlayerControl:popupMode:)]) {
        [self.delegate audioPlayerControl:self popupMode:AudioPlayerPopupList];
    }
}
